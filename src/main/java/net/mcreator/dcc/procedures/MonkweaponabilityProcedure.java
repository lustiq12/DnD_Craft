...
public class MonkweaponabilityProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity sourceentity) {
		...
		if ((sourceentity instanceof Player _plr ? _plr.experienceLevel : 0) > 79) {
			if ((sourceentity.getData(DccModVariables.PLAYER_VARIABLES).Class_Variable).equals("Monk")) {
				if ((sourceentity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == DccModItems.BATTLE_STAFF.get()
