	...
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			if (!world.getLevelData().getGameRules().getBoolean(GameRules.RULE_KEEPINVENTORY)) {
				...
				if (entity instanceof Player _player) {
					ItemStack _setstack = new ItemStack(DccModItems.SPELLBOOK.get()).copy();
					_setstack.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
				}
				if (entity instanceof Player _player) {
					ItemStack _setstack = new ItemStack(DccModItems.ANIMAL_FAT.get()).copy();
				...
				if (entity instanceof Player _player) {
					ItemStack _setstack = new ItemStack(DccModItems.POTIONOFSTRONGHEALING.get()).copy();
					_setstack.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
				}
			}
